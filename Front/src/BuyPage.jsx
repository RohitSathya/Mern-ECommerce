...
export default function BuyPage({data,data2,func}) {
  ...
  useEffect(()=>{
    ...
    useEffect(()=>{
      ...
      async function getad(){
        ...
        const response=await axios.get(`http://localhost:8080/product/getaddress/${parse._id}`)
    ...
    async function address(){
      if(mf!==0){
        const userdetail=localStorage.getItem('userdetail')
        const parse=JSON.parse(userdetail)
        const upad=await axios.put(`http://localhost:8080/product/updateaddress/${parse._id}/${name}/${pno}/${Landmark}/${pincode}/${area}`)
        sfl(fl+1)
      }
      else{
        const userdetail=localStorage.getItem('userdetail')
        ...
        const response=await axios.post('http://localhost:8080/product/address',{country:country,name:name,phoneno:pno,pincode:pincode,area:area,landmark:Landmark,uid:parse._id})
 ...
 function pay(){
   ...
   async function dcart(){
     ...
     const res=await axios.delete(`http://localhost:8080/product/dcart/${parse._id}`)
     const {message}=res.data
     if(message=='s'){
      dispatch(fastcount())
      dispatch(postorder(ord))
      const res=await axios.post(`http://localhost:8080/product/order`,{ord})
